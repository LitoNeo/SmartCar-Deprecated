...
namespace FAST_NDT{
	void LidarMapping::param_callback(config_msgs::ConfigNdtMapping msg) {
		...
		trans_eps = msg.trans_epsilon;
		max_iter = msg.max_iterations;
		voxel_leaf_size = msg.leaf_size;
		min_scan_range = msg.min_scan_range;
		max_scan_range = msg.max_scan_range;
		min_add_scan_shift = msg.min_add_scan_shift;

		std::cout << "Set param_callback" << std::endl;
